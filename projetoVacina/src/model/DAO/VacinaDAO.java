...
public class VacinaDAO {
	...
	public Vacina inserir(Vacina novaVacina) {
		...
		try {
			...
			query.setObject(4, novaVacina.getDataInicioPesquisa());
...
// UPDATE vacina
	public boolean atualizar(Vacina vacinaEditada) {
		...
		try {
			...
			query.setString(2, vacinaEditada.getPaisDeOrigem());
			query.setInt(3, vacinaEditada.getEstagioPesquisa().getValor());
			query.setObject(4, vacinaEditada.getDataInicioPesquisa());
	...
	private Vacina converterResultSetParaEntidade(ResultSet resultado) throws SQLException {
		...
		vacinaConsultada.setEstagioPesquisa(EstagioPesquisa.getEstagioPesquisaPorValor(resultado.getInt("idtipofasevacina")));
		vacinaConsultada.setDataInicioPesquisa(LocalDateTime.parse(resultado.getString("data_inicio_da_pesquisa"),
