...
public class Executavel {
	public static void main(String[] args) {
		...
		Pessoa pessoa2 = new Pessoa(null, "Joao Gomes", TipoPessoa.PUBLICO_GERAL, "10044587954",
				LocalDateTime.of(2002, 11, 5, 0, 0), 'M');
		...
		// ATUALIZAR PESSOA
		  ...
		  for (Pessoa pessoa : dbaDePessoas.consultarTodos()) { System.out.println(pessoa.getNome()); }
		  ...
		  Vacina vacina1 = new Vacina(null, "COVID",
		  
		 EstagioPesquisa.APLICACAO_EM_MASSA, "Brasil", LocalDateTime.now(),
		  dbaDePessoas.consultarPorId(2)); Vacina vacina2 = new Vacina(null, "H1N1",
		  EstagioPesquisa.INICIAL, "Estados Unidos", LocalDateTime.now(),
		  dbaDePessoas.consultarPorId(1)); Vacina vacina3 = new Vacina(null, "DENGUE",
		  EstagioPesquisa.TESTES, "Brasil", LocalDateTime.now(),
		  dbaDePessoas.consultarPorId(3)); VacinaDAO dbaDeVacina = new VacinaDAO();
		  
		  dbaDeVacina.inserir(vacina1); dbaDeVacina.inserir(vacina2);
		  dbaDeVacina.inserir(vacina3);
		  
		  if (vacina3.getId() != null) { System.out.println("Nova vacina cadastrada!");
		  } else { System.out.println("Erro ao cadastrar nova vacina!"); }
		  
		 // Atualizar Vacina
		  Vacina vacinaExistente = dbaDeVacina.consultarPorId(1);
		 vacinaExistente.setPesquisador(dbaDePessoas.consultarPorId(4));
		  vacinaExistente.setPaisDeOrigem("Brasil"); boolean atualizou =
		  dbaDeVacina.atualizar(vacinaExistente);
		  
		 if (atualizou) { System.out.println("Vacina foi atualizada!"); } else {
		  System.out.println("Erro ao atualizar vacina!"); }
		  
		  // Excluir Vacina
		 
		 if (dbaDeVacina.excluir(2)) {
		  System.out.println("Vacina foi excluida!"); }else
